<?php
declare(strict_types=1);
require_once __DIR__ . '/../../../base-url.php';
rh_start_output_rewrite();
?>
<!DOCTYPE html><html lang="EN" class="js-on"><head>

<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/CSS/sc_common.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/CSS/gl_header_localize.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/CSS/sc_common_us.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Sites/com/cmn_us_v1/css/common.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Sites/com/cmn_us_v1/css/gl_header.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Sites/com/cmn_us_v1/css/component.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Sites/com/cmn_us_v1/css/option.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Sites/com/cmn_us_v1/css/ex_component.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_g_header_footer/css/template.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_g_header_footer/css/conflict_clear.css">
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/Lib/fancybox-v1.4.3/CSS/jquery.fancybox-1.3.4.css">

<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/css/template-global-addition2024.css">


    
    

<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Directory | Global | Ricoh</title>
    <meta name="Description" content="[Ricoh Global Official Website] Ricoh business directory, listing sales and services offices in Americas, Europe and Asia Pacific/China.">
<meta property="og:title" content="Directory | Global | Ricoh">
    <meta property="og:image" content="https://www.ricoh.com/-/media/Ricoh/Common/cmn_v3/img/og-image.webp?rev=549e30934785489fbd556ee2e4c3b238&amp;hash=9DB561AD22E96D26590C1574F15A7B50">
    <meta property="og:type" content="website">
    <meta property="og:description" content="Ricoh business directory, listing sales and services offices in Americas, Europe and Asia Pacific/China.">
<meta property="og:url" content="https://www.ricoh.com/about/location">
    <meta property="og:site_name" content="Ricoh Global Website">
    <meta property="og:country-name" content="Global">

    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:site" content="@ricoh">

<link rel="canonical" href="">


<link rel="icon" type="image/vnd.microsoft.icon" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/media/ScAssets/System/Images/favicon.ico">
<link rel="apple-touch-icon" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/media/ScAssets/System/Images/apple_icon.png">




<script type="text/javascript" src="/ricoh-clone/ricoh_offline/assets/cdn.fonts.net/kit/3c059441-5218-4b9b-974d-a8185a3bd584/3c059441-5218-4b9b-974d-a8185a3bd584_enhanced.js" async=""></script>
<link rel="stylesheet" type="text/css" href="/ricoh-clone/ricoh_offline/assets/cdn.fonts.net/kit/3c059441-5218-4b9b-974d-a8185a3bd584/3c059441-5218-4b9b-974d-a8185a3bd584_enhanced.css">
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/Lib/jquery.min.js"></script>
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/Lib/jquery.bxslider.min.js" defer=""></script>




<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/Lib/bxslider/jquery.bxslider.css">


<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/js/jquery-match-height.js" defer=""></script>
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/js/solution-product-template.js" defer=""></script>
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/Scripts/dist/lib/vendor.js"></script>


<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/JS/common.js" defer=""></script>
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/css/import.css">


<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/css/solution-product-template.css">





<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/js/globalnavi.js" defer=""></script>
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/Lib/jquery.tile.js" defer=""></script>


<script type="text/javascript" src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/modal-for-video/jquery.magnific-popup.min.js" defer=""></script>
<script type="text/javascript" src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/modal-for-video/jquery.modal-for-video.js" defer=""></script>
<script type="text/javascript" src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/modal-for-video/youtube_modal_setting.js" defer=""></script>
<link rel="stylesheet" type="text/css" media="all" href="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/modal-for-video/magnific-popup.css" charset="UTF-8">


<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/js/common.vanilla.js" defer=""></script>



<style>
  .c-button-a--map .c-button-a__text::before {
    content: "";
    -webkit-mask-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 150 150"><g><path d="M75,15.9c22.76,0,41.22,18.45,41.22,41.22s-41.22,76.99-41.22,76.99c0,0-41.22-54.22-41.22-76.99S52.24,15.9,75,15.9ZM75,80.72c13.04,0,23.6-10.57,23.6-23.6s-10.57-23.6-23.6-23.6-23.6,10.57-23.6,23.6,10.57,23.6,23.6,23.6Z"/></g></svg>');
    mask-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 150 150"><g><path d="M75,15.9c22.76,0,41.22,18.45,41.22,41.22s-41.22,76.99-41.22,76.99c0,0-41.22-54.22-41.22-76.99S52.24,15.9,75,15.9ZM75,80.72c13.04,0,23.6-10.57,23.6-23.6s-10.57-23.6-23.6-23.6-23.6,10.57-23.6,23.6,10.57,23.6,23.6,23.6Z"/></g></svg>')
  }

  #section07 {
    padding-top: 0;
  }

  #section04 {
    border-top: none;
  }
</style>
    
    
    

</head>
<body>
    <div id="wrapper">
        
        
        <header class="gl-header -horizontal">
    <span class="hide"></span>
    <div class="gl-header_inner">
        <div class="gl-header_logo">
            <a href="../../"><img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_g_header_footer/img/logo/logo.svg" alt="RICOH imagine. change." width="125" height="43"></a>
        </div>

        <div class="gl-header_menu">
            <nav class="gl-menu js-global-menu" data-label="Menu" aria-label="Main navigation">
                <div class="gl-menu_content fn-menu_hamburger-menu">
                        <div class="gi-menu-globe">
                            <a href="../../gateway/" class="gi-menu_globe-btn">
                                <span class="gi-menu_globe-txt">Country/Area Selector</span>
                                <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_g_header_footer/img/icon/globe.svg" class="gi-menu_globe-icon" alt="Change Country/Area">
                            </a>
                        </div>
                                                            <a class="hide" href="#contents">Skip to main content</a>
                    <span class="hide">First level navigation</span>
                    <ul class="gl-menu_main">
<li>
<a href="../" class="{37791300-1D98-4601-A34E-687E71DCCA2E} gl-menu_link  fn-menu_main fn-current" aria-current="page"><span>About RICOH</span></a>

</li>
<li>
<a href="../../IR/" class="{419FD399-BE45-4B6F-A170-DDF4C563F180} gl-menu_link  fn-menu_main"><span>Investor Relations</span></a>

</li>
<li>
<a href="../../sustainability/" class="{1B1686A6-FFF5-472A-A359-7E48900444A1} gl-menu_link  fn-menu_main"><span>Sustainability</span></a>

</li>
<li>
<a href="../../products/" class="{C5764788-D65B-4D47-84F6-9EED5B48E30E} gl-menu_link  fn-menu_main"><span>Products</span></a>
</li><li>
<a href="../../technology/" class="{269944AA-F9F1-450F-808A-5A0A25693389} gl-menu_link  fn-menu_main"><span>Technology</span></a>

</li>
<li>
<a href="../../support/" class="{CD078A07-1565-4DCA-8688-B247492DCA7B} gl-menu_link  fn-menu_main"><span>Support &amp; Downloads</span></a>
</li><li>
<a href="../../news/" class="{0D2ED999-CA97-40DF-A541-17D96E4A0DF5} gl-menu_link  fn-menu_main"><span>News</span></a>

</li>
                    </ul>
                </div>

                <template class="fn-menu_tpl-hamburger">
                    <button class="gl-menu_toggle-btn" data-alt-open="Open" data-alt-close="Close"><span class="hide">Menu</span></button>
                </template>
                <template class="fn-menu_tpl-back">
                    <button class="gl-menu_back-btn">Home</button>
                </template>
                <template class="fn-menu_tpl-close">
                    <button class="gl-menu_close-btn" data-alt-close="Close"></button>
                </template>
            </nav>
        </div>

        <div class="gl-header_utilities">
                                                                            </div>
    </div>
</header>

        <div id="contents" class="">
            <div id="Main">
                
    <div class="breadcrumb breadcrumb--borderBottom">
        <div class="breadcrumb__container">
            <div class="breadcrumb__inner">
                <ul class="breadcrumb__list">
<li>
  <a href="../../">Home</a>
</li>
<li>
  <a href="../">About RICOH</a>
</li>                                <li class="act">
                                    <span>Directory</span>
                                </li>
                </ul>
            </div>
        </div>
    </div>


                
                
<div class="c-container">
        <div class="c-container">
        <div data-globalnav="area" class="c-globalNav-area" id="localnavi">
            <div class="c-globalNav" data-globalnav="target">
                <div class="c-globalNav__container">
                    <div class="c-globalNav__inner">
                        <div class="c-globalNav__box">
                            <div class="c-globalNav__titleWrapper">
                                        <a href="../" target="_self" class="c-globalNav__title">
                                            <span class="c-globalNav__titleText">About RICOH</span>
                                        </a>
                            </div>
                            
                                <div class="c-globalNav__button">
                                    <button class="c-globalNav__buttonArrow" data-globalnav="fadeInButton" aria-expanded="false" aria-controls="globalNavControlsId"></button>
                                </div>
                                <div class="c-globalNav__content" data-globalnav="content" id="globalNavControlsId">
                                    <div class="c-globalNav__fadeInner">
                                        <div class="c-globalNav__linkArea">
<ul class="c-globalNav__list">
  <li class="c-globalNav__item" data-drawer-button="01">
    <button class="c-globalNav__link" aria-expanded="false" aria-controls="globalNavDrawerControlsId-01">Vision</button>
    <div class="c-globalNav-drawer__box" data-drawer-inner="01" id="globalNavDrawerControlsId-01">
      <ul class="c-globalNav-drawer__list">
        <li class="c-globalNav-drawer__item"><a href="../to-our-stakeholders/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">To Our Stakeholders</span></a></li>
        <li class="c-globalNav-drawer__item"><a href="../ricoh-way/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Corporate Philosophy</span></a></li>
      </ul>
    </div>
  </li>
  <li class="c-globalNav__item" data-drawer-button="02">
    <button class="c-globalNav__link" aria-expanded="false" aria-controls="globalNavDrawerControlsId-02">Company Overview</button>
    <div class="c-globalNav-drawer__box" data-drawer-inner="02" id="globalNavDrawerControlsId-02">
      <ul class="c-globalNav-drawer__list">
        <li class="c-globalNav-drawer__item"><a href="../facts/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Company Data</span></a></li>
        <!-- li class="c-globalNav-drawer__item"><a href="../business-overview/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Business</span></a></li -->
        <li class="c-globalNav-drawer__item"><a href="../facts/board/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">The Board</span></a></li>
        <li class="c-globalNav-drawer__item"><a href="../organization/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Organization</span></a></li>
        <li class="c-globalNav-drawer__item"><a href="" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Directory</span></a></li>
        <li class="c-globalNav-drawer__item"><a href="../global-network/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Global Network</span></a></li>
        <li class="c-globalNav-drawer__item"><a href="../history/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Company History</span></a></li>
        <li class="c-globalNav-drawer__item"><a href="../at-a-glance/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Ricoh at a Glance</span></a></li>
      </ul>
    </div>
  </li>
  <li class="c-globalNav__item">
    <a href="../integrated-report/" class="c-globalNav__link">Integrated Report</a>
  </li>
  <li class="c-globalNav__item" data-drawer-button="03">
    <button class="c-globalNav__link" aria-expanded="false" aria-controls="globalNavDrawerControlsId-03">Advertising / Publications</button>
    <div class="c-globalNav-drawer__box" data-drawer-inner="03" id="globalNavDrawerControlsId-03">
      <ul class="c-globalNav-drawer__list">
        <li class="c-globalNav-drawer__item"><a href="../advertisement/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Advertising</span></a></li>
        <li class="c-globalNav-drawer__item"><a href="../video/" class="c-globalNav-drawer__link"><span class="c-globalNav-drawer__text">Corporate Videos &amp; Publications</span></a></li>
      </ul>
    </div>
  </li>
  <li class="c-globalNav__item">
    <a href="../jobs/" class="c-globalNav__link">Careers</a>
  </li>
</ul>                                                                                    </div>
                                    </div>
                                </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="c-h1-b" id="freepagetitle">
        <div class="c-margin-b">
            <div class="c-margin-b__inner">
                    <h1 class="c-h1-b__text">
                        Directory
                    </h1>
            </div>
        </div>
    </div>

    <div class="fxb-section">
<section class="c-section " id="section07">
    <div class="f-child">
        <div class="c-margin-b">
            <div class="c-margin-b__inner">
                <div class="c-text-a">
  <p class="c-text-a__text">
    The Ricoh Group’s global network provides products and solutions to about 200 countries and regions around the world.
  </p>
</div>            </div>
        </div>
    </div>
</section><section class="c-ancLink-a">
  <div class="c-margin-a">
    <div class="c-margin-a__inner">
      <ul class="c-ancLink-a__list">
        <li class="c-ancLink-a__item">
          <a href="#section01" class="c-ancLink-a__link">Office (Ricoh Company, Ltd.)</a>
        </li>
        <li class="c-ancLink-a__item">
          <a href="#section02" class="c-ancLink-a__link">R&amp;D and Manufacturing (Ricoh Company, Ltd.)</a>
        </li>
        <li class="c-ancLink-a__item">
          <a href="#section03" class="c-ancLink-a__link">Sales and Services</a>
        </li>
        <li class="c-ancLink-a__item">
          <a href="#section04" class="c-ancLink-a__link">Development, Design, and Production</a>
        </li>
      </ul>
    </div>
  </div>
</section>

<section class="c-section " id="section01">
    <div class="f-child">
        <div class="c-margin-b">
            <div class="c-margin-b__inner">
                <div class="c-h2-a">
                    <h2 class="c-h2-a__text">Office (Ricoh Company, Ltd.)</h2>
                </div>
            </div>
        </div>
    </div>
    <div class="c-accordion-b">
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="01">
                                        <span>Headquarters</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="01">
<div class="c-text-a">
  <p class="c-text-a__text">
    3-6, Nakamagome 1-chome, Ohta-ku, Tokyo 143-8555 Japan
  </p>
  <p class="c-text-a__text">
    +81 3-3777-8111
  </p>
</div>
<div class="c-link-a-list u-mt-0">
  <div class="c-link-a-list__inner">
    <a href="../../" class="c-link-a icon-hidden">
      <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/arrow-right-primary-color.svg" alt="">
      <span class="c-link-a__text">Website</span>
    </a>
  </div>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text">
    Toei Subway Asakusa Line, Magome Station; 8 minute walk<br>
    Go out Exit A1 (Kanjo 7), and turn right.
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/eYkxni2ibHMAuc3F9" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="02">
                                        <span>Yokohama Nakamachidai Office</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="02">
<div class="c-text-a">
  <p class="c-text-a__text">
    16-1, Shinei-cho, Tsuzuki-ku, Yokohama, Kanagawa 224-0035 Japan
  </p>
  <p class="c-text-a__text">
    +81 50-3817-4410
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/7QGaL7ii9Jns2SXt5" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="03">
                                        <span>RICOH Eco Business Development Center</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="03">
<div class="c-text-a">
  <p class="c-text-a__text">
    1-10 Komakado, Gotemba, Shizuoka 412-0038 Japan
  </p>
  <p class="c-text-a__text">
    +81 50-3817-5800
  </p>
</div>
<div class="c-link-a-list u-mt-0">
  <div class="c-link-a-list__inner">
    <a href="../../sustainability/environment/eco_business_center/" class="c-link-a icon-hidden">
      <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/arrow-right-primary-color.svg" alt="">
      <span class="c-link-a__text">Website</span>
    </a>
  </div>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    By Car
  </p>
  <p class="c-text-a__text">
    Leave the Tomei Expressway at the Gotemba IC and take the R138 in the direction of Lake Yamanaka.<br>
    Turn left at the R246 crossing and drive towards Numazu.<br>
    Turn right at the Yabai crossing to Komakado Industrial Estate.<br>
    Leave the Tomei Expressway at the Susono IC and take the R246 bypass route toward Tokyo.<br>
    Turn left at the Yabai crossing to Komakado Industrial Estate.
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    By Train
  </p>
  <p class="c-text-a__text">
    ・Via JR Tokaido Shinkansen line<br>
    Get off at Mishima station. Either take a taxi or change to the JR Tokaido line to go to Numazu Station.<br>
    Then change to the JR Gotemba line and get off at Fujioka Station.
  </p>
  <p class="c-text-a__text">
    ・Via JR Tokai, Tokaido line<br>
    From Tokyo, change to the JR Gotemba line at Kozu Station and get off at Gotemba or Fujioka Station.<br>
    From Shizuoka, change to the JR Gotemba line at Numazu Station and get off at Fujioka Station.
  </p>
  <p class="c-text-a__text">
    ・Via Odakyu line<br>
    Get off at Shinmatsuda Station. Walk to the JR Gotemba line at Matsuda Station. Get off the Gotemba line at Gotemba or Fujioka Station. Or take the Resort Super Express Asagiri from Shinjuku Station to travel directly to Gotemba Station.
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    Express Bus
  </p>
  <p class="c-text-a__text">
    Get off at Gotemba Station.
  </p>
  <ul class="c-attention-a__list">
    <li class="c-attention-a__item">
      <div class="c-attention-a__marker">
        <span>※</span>
      </div>
      <div class="c-attention-a__text">Take a taxi from Gotemba or Fujioka Station (see below).</div>
    </li>
  </ul>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    Taxi
  </p>
  <p class="c-text-a__text">
    Get off at Gotemba Station.<br>
    Five to ten minutes from Fujioka Station (No taxi rank at the station. Call a taxi by telephone.)
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/4tKFXbaZbjWYiMRo7" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </section>

<section class="c-section " id="section02">
    <div class="f-child">
        <div class="c-margin-b">
            <div class="c-margin-b__inner">
                <div class="c-h2-a">
                    <h2 class="c-h2-a__text">R&amp;D and Manufacturing (Ricoh Company, Ltd.)</h2>
                </div>
            </div>
        </div>
    </div>
    <div class="c-accordion-b">
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="01">
                                        <span>Atsugi Plant</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="01">
<div class="c-text-a">
  <p class="c-text-a__text">
    1005 Shimo-Ogino, Atsugi, Kanagawa 243-0298, Japan
  </p>
  <p class="c-text-a__text">
    +81 46-241-1511
  </p>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text">
    Odakyu Line, Hon-Atsugi Station
  </p>
  <p class="c-text-a__text u-font-bold">
    By Bus
  </p>
  <p class="c-text-a__text">
    -Exit station using the North Exit. From bus stop #1, take a Kanagawa Chuo Kotsu (Kanachu) bus for Atsugi Kyodo Hakubutsukan or Tobio-danchi via Kanagawa Institute of Technology. Get off at the Ricoh-mae bus stop after 20 to 30 minutes.
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    By Taxi
  </p>
  <p class="c-text-a__text">
    -Exit station using the North Exit. Take a taxi to the Atsugi Plant in Ogino.
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://www.google.com/maps/search/%E3%83%AA%E3%82%B3%E3%83%BC+%E5%8E%9A%E6%9C%A8%E4%BA%8B%E6%A5%AD%E6%89%80" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="03">
                                        <span>Numazu South Plant</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="03">
<div class="c-text-a">
  <p class="c-text-a__text">
    16-1 Hontamachi, Numazu, Shizuoka 410-8505, Japan
  </p>
  <p class="c-text-a__text">
    +81 55-920-1000
  </p>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    By Bus
  </p>
  <p class="c-text-a__text">
    -JR Tokaido-Sanyo Shinkansen, Mishima Station; Exit station using the North Exit. Take a taxi for Ricoh in Honda-machi (about 20 minutes).
  </p>
  <p class="c-text-a__text">
    -JR Tokaido Line, Numazu Station; Exit station using the North Exit. Take a taxi for Ricoh in Honda-machi (about 5 minutes) or walk (about 20 minutes).
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    By Car
  </p>
  <p class="c-text-a__text">
    -Exit the Tomei Expressway at the Numazu IC. Take National Route 1 in the direction of Shizuoka/Fuji (about 15 minutes).
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/tUoPgUmukmtft5T39" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="04">
                                        <span>Fukui Plant</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="04">
<div class="c-text-a">
  <p class="c-text-a__text">
    64-1 Ohmi, Sakai-cho, Sakai, Fukui 919-0547, Japan
  </p>
  <p class="c-text-a__text">
    +81 776-72-2700
  </p>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text">
    North Plant
  </p>
  <p class="c-text-a__text u-font-bold">
    By Train
  </p>
  <p class="c-text-a__text">
    -JR Hokuriku Line, Awara-onsen Station; Take a taxi from the station (about 15 minutes or 5 km)
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text">
    Reference
  </p>
  <p class="c-text-a__text u-font-bold">
    By Car
  </p>
  <p class="c-text-a__text">
    40 minutes from JR Fukui Station
  </p>
  <p class="c-text-a__text">
    20 minutes from Maruoka IC
  </p>
  <p class="c-text-a__text">
    60 minutes from Komatsu Airport
  </p>
  <p class="c-text-a__text u-font-bold">
    By Train
  </p>
  <p class="c-text-a__text">
    80 minutes from JR Tokaido-Sanyo Shinkansen Maibara Station
  </p>
  <p class="c-text-a__text">
    4 hours from Tokyo
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold">
    By Car
  </p>
  <p class="c-text-a__text">
    15 minutes (13 km) from Hokuriku Highway Maruoka ICExit the expressway at the Maruoka IC and go straight. Go under the overpass and immediately turn right. At the Ipponden intersection, turn left and go straight. Cross the bridge over the JR line, and continue through the Higashi-Nagata intersection and Keifuku Line railroad crossing. At the Nishi-Nagata intersection, turn right and go straight for about 4 km. The Fukui Plant will be on your left.
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/de4ThU6fE3nrHtKA9" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="05">
                                        <span>Service Parts Center</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="05">
<div class="c-text-a">
  <p class="c-text-a__text">
    235-6 Naganuma, Atsugi, Kanagawa 243-0024, Japan
  </p>
  <p class="c-text-a__text">
    +81 46-229-1211
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/tUh9TV6GC2nk7LdC6" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="06">
                                        <span>Kanazawa Office</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="06">
<div class="c-text-a">
  <p class="c-text-a__text">
    Kanazawa Techno Park
  </p>
  <p class="c-text-a__text">
    2-3 Hokuyodai, Kanazawa, Ishikawa, 920-0177, Japan
  </p>
  <p class="c-text-a__text">
    +81 76-258-7012
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="07">
                                        <span>Ricoh Technology Center</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="07">
<div class="c-text-a">
  <p class="c-text-a__text">
    2-7-1 Izumi, Ebina, Kanagawa 243-0460, Japan
  </p>
  <p class="c-text-a__text">
    +81 46-236-2400
  </p>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text">
    Sotetsu Line Ebina Station West Exit : 10 minutes walk
  </p>
  <p class="c-text-a__text">
    Odakyu Line Ebina Station West Exit : 10 minutes walk
  </p>
  <p class="c-text-a__text">
    JR Sagami Line Ebina Station West Exit : 7 minutes walk
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/wfrkzNYgrctZJDMp8" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="08">
                                        <span>Ikeda Office</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="08">
<div class="c-text-a">
  <p class="c-text-a__text">
    13-1 Himemuro-cho, Ikeda, Osaka 563-8501, Japan
  </p>
  <p class="c-text-a__text">
    +81 72-753-1111
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/3uUDfPnFXZsgfM6F6" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="09">
                                        <span>General Electronics R&amp;D Center</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="09">
<div class="c-text-a">
  <p class="c-text-a__text">
    5-10 Yokarakami, Kumanodo, Takadate, Natori, Miyagi 981-1241 Japan
  </p>
  <p class="c-text-a__text">
    +81 22-386-2011
  </p>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text">
    Sendai Station or Sendai Airport
  </p>
  <p class="c-text-a__text u-font-bold">
    By Taxi
  </p>
  <p class="c-text-a__text">
    -Take a taxi in the direction of Sendai Minami interchange. The center will be a brick-colored building with the Ricoh logo on it about 1 km before the Sendai Minami IC on Route 286. The nearest subway station is Nagamachi-minami.
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/cdaMLgfVtTRDTj7Q7" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="10">
                                        <span>Ricoh Kawasaki Life Innovation Center</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="10">
<div class="c-text-a">
  <p class="c-text-a__text">
    3-25-22 Tonomachi, Kawasaki-ku Kawasaki, Kanagawa, 210-0821, Japan
  </p>
  <p class="c-text-a__text">
    +81-50-3534-6900
  </p>
</div>
<div class="c-h4-a">
  <h4 class="c-h4-a__text">Access</h4>
</div>
<div class="c-text-a">
  <p class="c-text-a__text">
    Keikyu Daishi Line Kojimashinden Station: 15 minutes walk
  </p>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://www.google.com/maps/search/%E3%83%AA%E3%82%B3%E3%83%BC%E5%B7%9D%E5%B4%8E%E3%83%A9%E3%82%A4%E3%83%95%E3%82%A4%E3%83%8E%E3%83%99%E3%83%BC%E3%82%B7%E3%83%A7%E3%83%B3%E3%82%BB%E3%83%B3%E3%82%BF%E3%83%BC" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="11">
                                        <span>Ricoh Institute of Sustainability and Business</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="11">
<div class="c-text-a">
  <p class="c-text-a__text">
    3-6, Nakamagome 1-chome, Ohta-ku, Tokyo 143-8555 Japan
  </p>
  <p class="c-text-a__text">
    +81 3-3777-8111
  </p>
</div>
<div class="c-link-a-list u-mt-0">
  <div class="c-link-a-list__inner">
    <a href="../facts/RISB/" class="c-link-a icon-hidden">
      <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/arrow-right-primary-color.svg" alt="">
      <span class="c-link-a__text">Website</span>
    </a>
  </div>
</div>
<div class="c-text-a">
  <div class="c-linkCard__item u-mt-32">
    <div class="c-linkCard__inner">
      <a href="https://maps.app.goo.gl/eYkxni2ibHMAuc3F9" class="c-button-a c-button-a--red c-button-a--arrow c-button-a--map c-button-a--iconWhite icon-hidden" target="_blank">
        <span class="c-button-a__text">Google Map</span>
      </a>
    </div>
  </div>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </section>

<section class="c-section c-section--gray" id="section03">
    <div class="f-child">
        <div class="c-margin-b">
            <div class="c-margin-b__inner">
                <div class="c-h2-a">
                    <h2 class="c-h2-a__text">Sales and Services</h2>
                </div>
            </div>
        </div>
    </div>
    <div class="c-accordion-b">
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="01">
                                        <span>Japan</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="01">
<div class="c-link-a-list">
  <div class="c-link-a-list__inner">
    <a href="https://www.ricoh.co.jp" class="c-link-a icon-hidden">
      <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/arrow-right-primary-color.svg" alt="">
      <span class="c-link-a__text">Ricoh Japan Corporation</span>
    </a>
  </div>
</div>
<div class="c-text-a u-mt-0">
  <p class="c-text-a__text u-font-bold">
    Regional headquarters
  </p>
  <p class="c-text-a__text">
    Shiba-Koen First Building, 3-8-2 Shiba, Minato-ku, Tokyo 105-8503 Japan
  </p>
  <p class="c-text-a__text">
    +81 50-3534-0800
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="02">
                                        <span>North America</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="02">
<div class="c-text-a">
  <a href="https://www.ricoh-usa.com/en" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh USA, Inc.</span>
  </a>
  <p class="c-text-a__text u-font-bold">
    Regional headquarters
  </p>
  <p class="c-text-a__text">
    300 Eagleview Boulevard, Exton, PA 19341, U.S.A.
  </p>
  <p class="c-text-a__text">
    +1 610-296-8000
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Americas Holdings,Inc.
  </p>
  <p class="c-text-a__text">
    300 Eagleview Boulevard, Suite 200 Exton, PA 19341, U.S.A.
  </p>
  <p class="c-text-a__text">
    +1 610-296-8000
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.ca/en-CA" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Canada, Inc.</span>
  </a>
  <p class="c-text-a__text">
    5560 Explorer Drive Suite 100, Mississauga, Ontario, L4W 5M3, Canada
  </p>
  <p class="c-text-a__text">
    +1 905-795-9659
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="03">
                                        <span>Latin America</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="03">
<div class="c-text-a">
  <a href="https://www.ricoh-usa.com/en" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Latin America, Inc.</span>
  </a>
  <p class="c-text-a__text u-font-bold">
    Regional headquarters
  </p>
  <p class="c-text-a__text">
    2700 South Commerce Parkway Suite 201, Weston, Florida 33331, U.S.A.
  </p>
  <p class="c-text-a__text">
    +1 754-314-3550
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-ar" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Argentina S.A.</span>
  </a>
  <p class="c-text-a__text">
    Av. Caseros 3039, Edificio Tesla II Piso 7° U2, C1264AAK, Ciudad Autonoma de Buenos Aires, Argentina
  </p>
  <p class="c-text-a__text">
    +54 11-4711-8000
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/pt-br" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Brasil, S.A.</span>
  </a>
  <p class="c-text-a__text">
    Avenida Ceci, 286, Centro Empresarial Tamboré, Barueri, Sao Paulo, Brasil CEP 06460-120
  </p>
  <p class="c-text-a__text">
    +55 11-2575-4400
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Chile S.A.</span>
  </a>
  <p class="c-text-a__text">
    Rosario Norte 532, Of. 301, Las Condes, Santiago, Chile Código Postal 7561185
  </p>
  <p class="c-text-a__text">
    +56 2-2367-7200
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-co" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Colombia S.A.</span>
  </a>
  <p class="c-text-a__text">
    Carrera 85K No. 46A-66 Edicifio #2 Of 502, Industrial Park San Cayetano, Bogotá, Colombia
  </p>
  <p class="c-text-a__text">
    +57 1-580-4477
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-cr" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Costa Rica S.A.</span>
  </a>
  <p class="c-text-a__text">
    Heredia, La Valencia Oficentro Tecnopark, costado Oeste de Autos Xiri, contiguo a McDonald's, San José, Costa Rica
  </p>
  <p class="c-text-a__text">
    +506 2210-9300
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-do" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Dominicana, S.A.</span>
  </a>
  <p class="c-text-a__text">
    Av. Pedro Henriquez Urena #142, Ens. La Esperilla, Santo Domingo, Republica Dominicana
  </p>
  <p class="c-text-a__text">
    +1 809-566-2026
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-sv" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh El Salvador, S.A. de C.V.</span>
  </a>
  <p class="c-text-a__text">
    55 Avenida Sur No. 153, entre Alameda Roosevelt y Avenida Olimpica, San Salvador, El Salvador
  </p>
  <p class="c-text-a__text">
    +503 2567-3500
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-gt" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Guatemala S.A.</span>
  </a>
  <p class="c-text-a__text">
    Avenida de Las Américas 24-04, Zona 13, Guatemala, Guatemala 01013
  </p>
  <p class="c-text-a__text">
    +502 2315-2000
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/en" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Latin America, Inc.</span>
  </a>
  <p class="c-text-a__text">
    2700 South Commerce Parkway Suite 201, Weston, Florida 33331, U.S.A.
  </p>
  <p class="c-text-a__text">
    +1 954-745-3300
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-mx" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Mexicana, S.A. de C.V.</span>
  </a>
  <p class="c-text-a__text">
    Edificio Corporativo Lomas Cantabria, Cerrada Palomas No. 22 Int 301, Colonia Reforma Social, C.P. 11650, Mexico D.F.
  </p>
  <p class="c-text-a__text">
    +52 55-5284-4600
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-pa" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Panamá, S.A.</span>
  </a>
  <p class="c-text-a__text">
    Boulevard Costa del Este, Edificio Financial Park, Piso 31, Ciudad de Panamá, Panamá
  </p>
  <p class="c-text-a__text">
    +507 831-6200
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-pe" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh del Perú S.A.C.</span>
  </a>
  <p class="c-text-a__text">
    Av. Victor Andrés Belaúnde 147, Torre Real 6 - Of. 402, San Isidro, Lima - Perú
  </p>
  <p class="c-text-a__text">
    +51 1-716-3600
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-americalatina.com/es-pr" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Puerto Rico, Inc.</span>
  </a>
  <p class="c-text-a__text">
    1510 Ave. Roosevelt, Suite 1200, Guaynabo, Puerto Rico, 00968
  </p>
  <p class="c-text-a__text">
    +1 787-641-4690
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="04">
                                        <span>Europe/Middle East/Africa</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="04">
<div class="c-text-a">
  <a href="https://www.ricoh-europe.com" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Europe PLC</span>
  </a>
  <p class="c-text-a__text u-font-bold">
    Regional headquarters
  </p>
  <p class="c-text-a__text">
    20 Triton Street, London. NW1 3BF, UK
  </p>
  <p class="c-text-a__text">
    +44 20-7465-1084
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.at" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Austria GmbH</span>
  </a>
  <p class="c-text-a__text">
    Thomas-Klestil-Platz 13, Wien 1030, Austria
  </p>
  <p class="c-text-a__text">
    +43 5-74-264-88650
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.be" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Belgium N.V.</span>
  </a>
  <p class="c-text-a__text">
    Medialaan 28 A, 1800 Vilvoorde, Belgium
  </p>
  <p class="c-text-a__text">
    +32 2-558-2211
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Czech Republic s.r.o.
  </p>
  <p class="c-text-a__text">
    Jihlavská 1558/21, 140 00 Praha 4, Czech Republic
  </p>
  <p class="c-text-a__text">
    +420 271-123-113
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.dk" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Denmark A/S</span>
  </a>
  <p class="c-text-a__text">
    Delta Park 37, 2665 Vallensbæk Strand, Denmark
  </p>
  <p class="c-text-a__text">
    +45 70-10-67-68
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.de" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Deutschland GmbH</span>
  </a>
  <p class="c-text-a__text">
    Vahrenwalder Str. 315 30179 Hannover, Germany
  </p>
  <p class="c-text-a__text">
    +49 511-6742-4000
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.es" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh España S.L.U.</span>
  </a>
  <p class="c-text-a__text">
    Avda. Via Augusta, 71-73, 08174 Sant Cugat del Vallés, Barcelona, Spain
  </p>
  <p class="c-text-a__text">
    +34 902-700-000
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Europe (Netherlands) B.V.
  </p>
  <p class="c-text-a__text">
    “Nieuw Kronenburg” Prof. W.H. Keesomlaan 1, 1183 DJ, Amstelveen, The Netherlands
  </p>
  <p class="c-text-a__text">
    +31 20-5474-111
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.fi" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Finland Oy</span>
  </a>
  <p class="c-text-a__text">
    Nittytaival 13, FI-02200 Espoo, Finland
  </p>
  <p class="c-text-a__text">
    +358 207-370-300
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.fr" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh France S.A.S.</span>
  </a>
  <p class="c-text-a__text">
    7-9 avenue Robert Schuman - Parc Icade Paris Orly Rungis, 94150 Rungis, France
  </p>
  <p class="c-text-a__text">
    +33 156-72-38-38
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.hu" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Hungary Kft.</span>
  </a>
  <p class="c-text-a__text">
    Bloc C and A Building, Puskas Tivadar Utca 14, 2040, Budaors, Hungary
  </p>
  <p class="c-text-a__text">
    +36 23-806-800
  </p>
</div>
<div class="c-text-a">
  <a href="https://ricoh-imaging.eu" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Imaging Europe S.A.S.</span>
  </a>
  <p class="c-text-a__text">7-9, avenue Robert Schuman, 94150 Rungis, France</p>
</div>
<div class="c-text-a">
  <a href="http://www.ricoh-international.com" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh International B.V.</span>
  </a>
  <p class="c-text-a__text">
    “Nieuw Kronenburg” Prof. W.H. Keesomlaan 1, 1183 DJ, Amstelveen, The Netherlands
  </p>
  <p class="c-text-a__text">
    +31 20-5474-300
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.ie" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Ireland Ltd.</span>
  </a>
  <p class="c-text-a__text">
    Unit 160, Lakeview Drive, Airside Business Park, Swords, Co Dublin, Ireland
  </p>
  <p class="c-text-a__text">
    +353 1895-3900
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.it" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Italia Srl.</span>
  </a>
  <p class="c-text-a__text">
    Viale Martesana 12 - 20055 Vimodrone (MI) - Italy
  </p>
  <p class="c-text-a__text">
    +39 02-9198-7100
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.lu" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Luxembourg DSC S.a.r.l.</span>
  </a>
  <p class="c-text-a__text">
    Atrium Business Park Z.I. Bourmicht 37, rue du Puits Romain L- 8070 Betrange Luxembourg
  </p>
  <p class="c-text-a__text">
    +352 33-58-88
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-me.com/en" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Middle East FZE</span>
  </a>
  <p class="c-text-a__text">
    P.O. Box 341307 Dubai Silicon Oasis-Silicon Park, Building A4, Second floor, office No. 217, Dubai-United Arab Emirates
  </p>
  <p class="c-text-a__text">
    +971 4-2990041
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.nl" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Nederland B.V.</span>
  </a>
  <p class="c-text-a__text">
    Hambakenwetering 1, 5231 DD's-Hertogenbosch, The Netherlands
  </p>
  <p class="c-text-a__text">
    +31 73-645-1111
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.no" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Norge A.S.</span>
  </a>
  <p class="c-text-a__text">
    Nydalen Park,Gjerdrums,vei 11, 0484 Oslo, Norway
  </p>
  <p class="c-text-a__text">
    +47 22-68-7800
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.pl" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Polska Sp.zo.o.</span>
  </a>
  <p class="c-text-a__text">
    Business Garden Warsaw, ul. Zwirki &amp; Wigury 18A, 02-092 Warsaw, Poland
  </p>
  <p class="c-text-a__text">
    +48 22-651-96-37
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.pt" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Portugal, Unipessoal, Lda</span>
  </a>
  <p class="c-text-a__text">
    Edificio Tower Plaza Via Engno Edgar Cardoso, n° 23-1°, 4400-676 Vila Nova de Gaia, Portugal
  </p>
  <p class="c-text-a__text">
    +351 2-2608-3800
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.ru" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Rus, Ltd.</span>
  </a>
  <p class="c-text-a__text">
    Kozhevnicheskaya Street 14, Building 5, Floor #8, Moscow, Russian Federation, 115114
  </p>
  <p class="c-text-a__text">
    +7 495-545-58-59
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.ch" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Schweiz AG</span>
  </a>
  <p class="c-text-a__text">
    Hertistrasse 2, CH-8304 Wallisellen, Switzerland
  </p>
  <p class="c-text-a__text">
    +41 44-832-32-32
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.sk" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Slovakia s.r.o.</span>
  </a>
  <p class="c-text-a__text">
    Vajnorská 100/B, 831 04 Bratislava, Slovakia
  </p>
  <p class="c-text-a__text">
    +421 2-326-306-20
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.co.za" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh South Africa (Pty) Ltd.</span>
  </a>
  <p class="c-text-a__text">
    Equites Park Meadowview Corner of Clulee Road and Gordon Avenue Meadowview Ext 132, Sandton, Gauteng, 2065, South Africa
  </p>
  <p class="c-text-a__text">
    +27 11-723-5000
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.se" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Sverige AB.</span>
  </a>
  <p class="c-text-a__text">
    Box 1536, 171 29 Solna, Sweden
  </p>
  <p class="c-text-a__text">
    +46 8-734-33-00
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.tr" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Turkey Baski Çözümleri Limited Şirketi</span>
  </a>
  <p class="c-text-a__text">
    Eski Büyükdere cad. No:74 Torun Center Projesi iç kapı no : 78-180-181-182-190 Mecidiyeköy / İstanbul, Turkey
  </p>
  <p class="c-text-a__text">
    +90 212-370-96-00
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.co.uk" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh UK Ltd.</span>
  </a>
  <p class="c-text-a__text">
    900 Pavilion Drive, Northampton Business Park, Northampton NN4 7RG U.K.
  </p>
  <p class="c-text-a__text">
    +44 1604-814900
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="05">
                                        <span>Asia Pacific/China</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="05">
<div class="c-text-a">
  <a href="https://www.ricoh-ap.com" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Asia Pacific Pte. Ltd.</span>
  </a>
  <p class="c-text-a__text u-font-bold">
    Regional headquarters
  </p>
  <p class="c-text-a__text">
    20W Pasir Panjang Road, #04-28 Mapletree Business City, Singapore 117439
  </p>
  <p class="c-text-a__text">
    +65 6830-5888
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Asia Pacific Operations Ltd.
  </p>
  <p class="c-text-a__text">
    15th Fl., One Kowloon, 1 Wang Yuen Street, Kowloon Bay, Hong Kong SAR, P.R. China
  </p>
  <p class="c-text-a__text">
    +852 2862-2888
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.au" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Australia Pty, Ltd.</span>
  </a>
  <p class="c-text-a__text">
    2 Richardson Place, North Ryde, NSW 2113 Australia
  </p>
  <p class="c-text-a__text">
    +61 2-8977-1111
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.cn" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh China Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    24th Fl, Lansheng Building, No.2-8, Huaihai Zhong Road, Shanghai, P.R. China 200021
  </p>
  <p class="c-text-a__text">
    +86 21-5238-0222
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.hk" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Hong Kong Ltd.</span>
  </a>
  <p class="c-text-a__text">
    20/F, One Kowloon, 1 Wang Yuen Street, Kowloon Bay, Hong Kong SAR, P.R. China
  </p>
  <p class="c-text-a__text">
    +852 2893-0022
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-imaging.com.cn/china" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Imaging China Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    23F Lansheng Building, No2-8 Huaihai Zhong Road, Huang Pu District, Shanghai 200021 China
  </p>
  <p class="c-text-a__text">
    +86 21-6404-8866
  </p>
</div>
<div class="c-text-a">
  <a href="http://www.ricoh-thermal.com.cn/" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh International (Shanghai) Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    Room 2901, Skyline Tower, No. 99, West Tianmu Road, Jing'an District, Shanghai, China, 200070
  </p>
  <p class="c-text-a__text">
    +86 21-6485-1058
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-korea.co.kr" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Korea Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    13F H Tower, 179, Bongeunsa-ro, Gangnam-gu, Seoul, Republic of Korea
  </p>
  <p class="c-text-a__text">
    +82 2-3677-1004
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.my" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh (Malaysia) Sdn.Bhd.</span>
  </a>
  <p class="c-text-a__text">
    LOCK &amp; STORE, Level 1 &amp; 3, No. 5, Jalan Penyair U1/44, Off Jalan Glenmarie, 40150 Shah Alam, Selangor Darul Ehsan, Malaysia
  </p>
  <p class="c-text-a__text">
    +603 5565-3888
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.co.nz" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh New Zealand Ltd.</span>
  </a>
  <p class="c-text-a__text">34 Sale Street, Auckland Central, Auckland 1010, New Zealand</p>
  <p class="c-text-a__text">+64 9-374-0701</p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.ph" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh (Philippines), Inc.</span>
  </a>
  <p class="c-text-a__text">1st and 2nd Floors, Kou Fu Packaging Building and Warehouse, Lot 1215, Governor's Drive, Bancal, Carmona City, Cavite, 4116, Philippines</p>
  <p class="c-text-a__text">+63 2-8865-9500</p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.sg" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh (Singapore) Pte Ltd.</span>
  </a>
  <p class="c-text-a__text">
    20W Pasir Panjang Road, #04-28 Mapletree Business City, Singapore 117439
  </p>
  <p class="c-text-a__text">
    +65 6474-0777
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.tw" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Taiwan Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    12F.-1, No.8, Sec. 7, Civic Blvd., Nangang District, Taipei City, 11562 Taiwan
  </p>
  <p class="c-text-a__text">
    +886 2-2654-8000
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.co.th" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh (Thailand) Ltd.</span>
  </a>
  <p class="c-text-a__text">
    341 Onnuj Road, Kwaeng Prawet, Prawet, Khet Bangkok 10250, Thailand
  </p>
  <p class="c-text-a__text">
    +66 2-088-8888
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh.com.vn" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Vietnam Company Limited</span>
  </a>
  <p class="c-text-a__text">
    Floor 17 F, E-Town Central, 11 Doan Van Bo Street Ward 12, District 4, Ho Chi Minh City, Viet Nam
  </p>
  <p class="c-text-a__text">
    +84 8-3528-5252
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </section>

<section class="c-section c-section--gray" id="section04">
    <div class="f-child">
        <div class="c-margin-b">
            <div class="c-margin-b__inner">
                <div class="c-h2-a">
                    <h2 class="c-h2-a__text">Development, Design, and Production</h2>
                </div>
            </div>
        </div>
    </div>
    <div class="c-accordion-b">
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="01">
                                        <span>Japan</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="01">
<div class="c-text-a">
  <a href="https://etria.global/en" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">ETRIA Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    1-2-5 Takashima, Nishi-ku, Yokohama, Kanagawa 220-0011, Japan
  </p>
  <p class="c-text-a__text">
    +81 50-3814-7000
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">ETRIA MANUFACTURING JAPAN Co., Ltd.</p>
  <p class="c-text-a__text">146-1 Nishisawada, Numazu, Shizuoka 410-0007, Japan</p>
  <p class="c-text-a__text">+81 50-3814-5442</p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-imaging.co.jp/english/" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Imaging Company, Ltd.</span>
  </a>
  <p class="c-text-a__text">
    16-1, Shinei-cho, Tsuzuki-ku, Yokohama, Kanagawa 224-0035 Japan
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Industry Company, Ltd.
  </p>
  <p class="c-text-a__text">
    1005 Shimo-Ogino, Atsugi, Kanagawa 243-0298, Japan
  </p>
  <p class="c-text-a__text">
    +81 46-243-1210
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricohelemex.co.jp/en" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Elemex Corporation</span>
  </a>
  <p class="c-text-a__text">
    3-69 Ida-cho, Okazaki, Aichi 444-8586, Japan
  </p>
  <p class="c-text-a__text">
    +81 564-23-5111
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-digitalpainting.com/en" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Digital Painting Company, Ltd.</span>
  </a>
  <p class="c-text-a__text">
    800-5 Oyama-machi, Machida-shi, Tokyo, 194-0212 Japan
  </p>
  <p class="c-text-a__text">
    +81-42-751-6110
  </p>
</div>
<div class="c-text-a">
  <a href="https://rpc.ricoh.com/en/" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">RICOH PFU COMPUTING Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    2-7-1 Izumi, Ebina, Kanagawa 243-0460, Japan
  </p>
  <p class="c-text-a__text">
    +81-50-3817-3333
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Hasama Ricoh, Inc.
  </p>
  <p class="c-text-a__text">
    86 Aza-Kitasanden, Sanuma, Hasama-cho, Tome, Miyagi 987-0511, Japan
  </p>
  <p class="c-text-a__text">
    +81 220-22-4431
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.pfu.ricoh.com/global" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">PFU Limited</span>
  </a>
  <p class="c-text-a__text u-font-bold">
    Headquarters
  </p>
  <p class="c-text-a__text">
    Nu 98-2 Unoke, Kahoku-shi, Ishikawa 929-1192 Japan
  </p>
  <p class="c-text-a__text">
    +81 76-283-1212
  </p>
  <p class="c-text-a__text u-font-bold">
    Yokohama Headquarters
  </p>
  <p class="c-text-a__text">
    YOKOHAMA i-MARK PLACE, 4-5 Minatomirai 4-chome, Nishi-ku, Yokohama-shi, Kanagawa 220-8567 Japan
  </p>
  <p class="c-text-a__text">
    +81 45-305-6000
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="02">
                                        <span>North America</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="02">
<div class="c-text-a">
  <a href="https://www.rei.ricoh.com" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Electronics, Inc.</span>
  </a>
  <p class="c-text-a__text">
    1125 Hurricane Shoals Road, Lawrenceville, GA 30043, U.S.A.
  </p>
  <p class="c-text-a__text">
    +1 770-338-7200
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">Ricoh Biosciences, Inc.</p>
  <p class="c-text-a__text">Science + Technology Park at Johns Hopkins 855 N. Wolfe St., Suite 631 Baltimore, MD 21205, U.S.A.</p>
  <p class="c-text-a__text">+1 443-869-5420</p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    ETRIA MANUFACTURING USA INC.
  </p>
  <p class="c-text-a__text">
    1005 Hurricane Shoals Rd, Lawrenceville, GA 30043, U.S.A.
  </p>
  <p class="c-text-a__text">
    +1 770-338-7202
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="03">
                                        <span>Europe/Middle East/Africa</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="03">
<div class="c-text-a">
  <a href="https://www.ricoh-thermal.com/fr" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh Industrie France S.A.S.</span>
  </a>
  <p class="c-text-a__text">
    144 Route de Rouffach 68920, Wettolsheim, France
  </p>
  <p class="c-text-a__text">
    +33 389-20-4000
  </p>
</div>
<div class="c-text-a">
  <a href="https://ukproducts.ricoh.com" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color.svg" alt="Open new window">
    <span class="c-link-a__text">Ricoh UK Products Ltd.</span>
  </a>
  <p class="c-text-a__text">
    Priorslee, Telford, Shropshire TF2 9NS, U.K.
  </p>
  <p class="c-text-a__text">
    +44 1952-29-0090
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="c-accordion-b__item">
            <div class="c-margin-b">
                <div class="c-margin-b__inner">
                    <div class="c-accordion-b__inner" data-accordion="item">
                        <div class="c-h3-a c-accordion-b__cell">
                            <h3 class="c-h3-a__text">
                                    <button data-accordion="trigger" aria-expanded="false" aria-controls="04">
                                        <span>Asia Pacific/China</span>
                                    </button>
                            </h3>
                        </div>
                        <div class="c-accordion-b__cell" data-accordion="inner" id="04">
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    ETRIA TRADING ASIA LIMITED
  </p>
  <p class="c-text-a__text">
    16F, Grandmark 8A-10 GranvileRoad, Tsimshatui, Kowloon, Hong Kong
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Toshiba Tec Information Systems (Shenzhen) Co., Ltd.
  </p>
  <p class="c-text-a__text">
    No7, 9, 28, DAYANG ROAD, FUHAI STREETS, BAOAN DISTRICT, SHENZHEN 518103, THE PEOPLE'S REPUBLIC OF CHINA
  </p>
  <p class="c-text-a__text">
    +86-755-2731-1901
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    ETRIA MANUFACTURING MALAYSIA SDN. BHD.
  </p>
  <p class="c-text-a__text">
    PLOT 111-A, HALA KAMPUNG JAWA 2, BAYAN LEPAS INDUSTRIAL ESTATE, 11900 PENANG, MALAYSIA
  </p>
  <p class="c-text-a__text">
    +60-4-6447790
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    ETRIA LOGISTICS &amp; PROCUREMENT H.K. LIMITED
  </p>
  <p class="c-text-a__text">
    Unit No.1703, 17th Floor, The Metropolis Tower, 10 Metropolis Drive, Hunghom, Kowloon, Hong Kong
  </p>
  <p class="c-text-a__text">
    +852-2356-5300
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Manufacturing (China) Ltd.
  </p>
  <p class="c-text-a__text">
    No.8, Liguang Road, Fenggang Town, Dongguan, Guangdong, China
  </p>
  <p class="c-text-a__text">
    +86 769-8681-0000
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Manufacturing (Thailand) Ltd.
  </p>
  <p class="c-text-a__text">
    7/152 Moo4 Tambol Mapyangporn, Amphur Pluakdaeng, Rayong 21140, Thailand
  </p>
  <p class="c-text-a__text">
    +66 38-650-839
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Imaging Products (Vietnam) Co., Ltd.
  </p>
  <p class="c-text-a__text">
    Plot A7, Sai Dong B Industrial Zone, Long Bien District, Hanoi, Vietnam
  </p>
  <p class="c-text-a__text">
    +84 24-3875-0418
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Ricoh Thermal Media Asia Pacific Private Limited
  </p>
  <p class="c-text-a__text">
    105, 1st Floor, Atrium 2, Chakala, Andheri-Kurla Road, Andheri (East), Mumbai 400059, India
  </p>
  <p class="c-text-a__text">
    +91 22-2832-6314 / 22-2830-6314
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">P.T. Ricoh Thermal Media East Asia Pacific</p>
  <p class="c-text-a__text">Marunda Center Blok E No. 43-45, Jl. Marunda Makmur, Bekasi Jawa Barat 17211, Indonesia</p>
  <p class="c-text-a__text">+62 21-2210-0350</p>
</div>
<div class="c-text-a">
  <a href="http://www.ricoh-thermal.com.cn" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Thermal Media (Wuxi) Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    No.1 Xinchang South Road, Country High Technology Industry Development Zone, Wuxi, Jiangsu, 214028, P. R. China
  </p>
  <p class="c-text-a__text">
    +86 510-8533-0555
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Shanghai Ricoh Digital Equipment Co., Ltd.
  </p>
  <p class="c-text-a__text">
    No.887, Jingang Road, Free Trade Zone, Pudong, Shanghai, P.R. China
  </p>
  <p class="c-text-a__text">
    +86 21-5854-9000
  </p>
</div>
<div class="c-text-a">
  <p class="c-text-a__text u-font-bold u-fc-cf142b">
    Shanghai Ricoh Office Equipment Co.,Ltd.
  </p>
  <p class="c-text-a__text">
    No.166, Meigui Road, Free Trade Zone, Pudong, Shanghai, P.R. China
  </p>
  <p class="c-text-a__text">
    +86 21-5048-0936
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.ricoh-rits.com" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Imaging Technology (Shanghai) Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    11th Floor Building B, No.1308 Lianhua Road, Minhang District, Shanghai, 201102, P.R.China
  </p>
  <p class="c-text-a__text">
    +86 21-5171-8992
  </p>
</div>
<div class="c-text-a">
  <a href="https://www.srcb-ricoh.com" class="c-link-a c-link-a--blank icon-hidden" target="_blank">
    <img src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_v3/img/svg/blank-primary-color" alt="Open new window">
    <span class="c-link-a__text">Ricoh Software Research Center (Beijing) Co., Ltd.</span>
  </a>
  <p class="c-text-a__text">
    3rd Floor East building, Genesis Beijing, No.8 Xinyuan South Road, Chaoyang District, Beijing, 100027, P.R.China
  </p>
  <p class="c-text-a__text">
    +86 10-8857-7878
  </p>
</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </section></div>
    
</div>

            </div>
        </div>
        
            <div class="breadcrumb breadcrumb--borderTop">
                <div class="breadcrumb__container">
                    <div class="breadcrumb__inner">
                        <ul class="breadcrumb__list">
<li>
  <a href="../../">Home</a>
</li>
<li>
  <a href="../">About RICOH</a>
</li>                            <li class="act">
                                <span>Directory</span>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>

        <footer class="gl-footer">
  <div class="gl-footer-link-area">
    <div class="gl-footer-link-area_inner">
      <div class="gl-footer-link-area-pc">
        <div class="gl-footer-link-area-pc_list">
          <div class="gl-footer-link-area-pc_item">
            <a href="../" class="gl-footer-link-area-pc_link-hdg">About RICOH</a>
            <ul class="gl-footer-link-area-pc_link-list">
              <li class="gl-footer-link-area-pc_link-item"><a href="../">Vision</a></li>
              <li class="gl-footer-link-area-pc_link-item"><a href="../">Company Overview</a></li>
              </ul>
          </div>
          <div class="gl-footer-link-area-pc_item">
            <a href="../../IR/" class="gl-footer-link-area-pc_link-hdg">Investor Relations</a>
            <ul class="gl-footer-link-area-pc_link-list">
              <li class="gl-footer-link-area-pc_link-item"><a href="../../IR/news/">IR News</a></li>
              <li class="gl-footer-link-area-pc_link-item"><a href="../../IR/events/investors-meeting/">Corporate Strategy Meeting</a></li>
              </ul>
          </div>
          <div class="gl-footer-link-area-pc_item">
            <a href="../../sustainability/" class="gl-footer-link-area-pc_link-hdg">Sustainability</a>
            <ul class="gl-footer-link-area-pc_link-list">
              <li class="gl-footer-link-area-pc_link-item"><a href="../../sustainability/environment/">Environment</a></li>
              <li class="gl-footer-link-area-pc_link-item"><a href="../../sustainability/society/">Society</a></li>
              </ul>
            </div>
          <div class="gl-footer-link-area-pc_item">
            <a href="../../technology/" class="gl-footer-link-area-pc_link-hdg">Technology</a>
            <ul class="gl-footer-link-area-pc_link-list">
              <li class="gl-footer-link-area-pc_link-item"><a href="../../technology/rd/">Development</a></li>
              <li class="gl-footer-link-area-pc_link-item"><a href="../../technology/tech/">Ricoh's Technology</a></li>
              </ul>
            </div>
          <div class="gl-footer-link-area-pc_item">
            <a href="../../news/" class="gl-footer-link-area-pc_link-hdg">News</a>
            <ul class="gl-footer-link-area-pc_link-list">
              <li class="gl-footer-link-area-pc_link-item"><a href="../../release/">News Release</a></li>
              <li class="gl-footer-link-area-pc_link-item"><a href="../../info/">Information</a></li>
              </ul>
          </div>
        </div>
      </div>
      <div class="gl-footer-link-area-sp">
        <ul class="gl-footer-link-area-sp_list">
          <li class="gl-footer-link-area-sp_item"><a href="../">About RICOH</a></li>
          <li class="gl-footer-link-area-sp_item"><a href="../../IR/">Investor Relations</a></li>
          <li class="gl-footer-link-area-sp_item"><a href="../../sustainability/">Sustainability</a></li>
          <li class="gl-footer-link-area-sp_item"><a href="../../technology/">Technology</a></li>
          <li class="gl-footer-link-area-sp_item"><a href="../../news/">News</a></li>
        </ul>
      </div>
    </div>
  </div>
  <div class="gl-footer-area-01">
    <div class="gl-footer-area-01_inner">
      <div class="gl-footer-pageTop">
        <a href="#top" class="gl-footer-pageTop_link js-top-button">Page Top</a>
      </div>
    </div><!-- /.gl-footer-area-01_inner -->
  </div><!-- /.gl-footer-area-01 -->
  <div class="gl-footer-area-02">
    <div class="gl-footer-area-02_inner">
      <div class="gl-footer-area-02_wrap">
        <span class="hide">ここからこのサイトのサブメニュー</span>
        <div class="gl-footer-utility">
          <ul class="gl-footer-utility_inner">
            <li class="gl-footer-utility_item"><a href="../../contact/" class="gl-footer-utility_link">Contact</a></li>
            <li class="gl-footer-utility_item"><a href="../../privacy/" class="gl-footer-utility_link">Privacy policy</a></li>
            <li class="gl-footer-utility_item"><a href="../../terms-of-use/" class="gl-footer-utility_link">Terms of use</a></li>
            <li class="gl-footer-utility_item"><a href="../../terms-of-use/accessibility/" class="gl-footer-utility_link">Accessibility</a></li>
            <li class="gl-footer-utility_item"><a href="../../sitemap/" class="gl-footer-utility_link">Sitemap</a></li>
            <li class="gl-footer-utility_item">
            </ul>
        </div><!-- /.gl-footer-utility -->
        <div class="gl-footer-copyright">
          <div class="gl-footer-copyright_inner">
            <small class="gl-footer-copyright_text">©&nbsp;Ricoh</small>
          </div>
        </div><!-- /.gl-footer-copyright -->
      </div><!-- /.gl-footer-area-02_wrap -->
    </div><!-- /.gl-footer-area-02_inner -->
  </div><!-- /.gl-footer-area-02 -->
</footer><!-- /.gl-footer -->
        <div class="cp-PageTop smoothScroll" style="display: none;">
            <a class="ver" href="#wrapper" aria-label="Page Top">PAGE TOP</a>
        </div>
        

    

<script type="text/javascript">
!(function () {
var G = (GALFSRAM = window.GALFSRAM || {});
G.mfx = G.mfx || {};
G.mfx.router_mode = "history";
})();
</script>
</div>


        

<!-- RICOH_PAGE_SCRIPTS -->
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/Lib/jquery.min.js"></script>
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/ScAssets/System/JS/common.js"></script>
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_g_header_footer/js/init.js"></script>
<script src="/ricoh-clone/ricoh_offline/assets/www.ricoh.com/-/Media/Ricoh/Common/cmn_g_header_footer/js/template.js"></script>
</body></html>